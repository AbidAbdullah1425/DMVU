...
async def download_m3u8(url, save_path):
    ...
    with open(save_path, "wb") as f:
        while downloaded_size < total_size:
            await asyncio.sleep(1)  # Asynchronously simulate downloading for 1 second
            ...
            # Yield progress update as a dictionary
            yield {
                "progress": progress,
                "percent": percent,
...
async def upload_file(client, message, file_path, user_id, user_name):
    ...
    with open(file_path, "rb") as file:
        while uploaded_size < total_size:
            ...
            # Update the progress message with the upload status
            progress_text = f"""
            video
            ...
            await asyncio.sleep(0.1)  # Give a slight delay to allow message edits
...
async def handle_dl_command(client, message):
    ...
    # Simulate download and send progress updates using async for
    ...
    # Once the download completes, update the message and start upload
    await progress_message.edit("Download complete! Now uploading the video...")

    # Upload the video (simulate upload progress)
    await upload_file(client, progress_message, save_path, user_id, user_name)

    # Once uploaded, notify the user and send the file as a document
